// File: my_orders_app.js - Phiên bản nâng cấp "lung linh"
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- CÁC HÀM CHÍNH ĐÃ ĐƯỢC NÂNG CẤP ---
    ...
    function populateWrappedData(customer) {
        ...
        // NÂNG CẤP: Dùng anime.js để đếm số thay vì gán trực tiếp
        ...
        if (totalOrdersEl) {
            ...
            anime({
                targets: orderCounter, value: totalOrders, round: 1, duration: 1500, easing: 'easeInOutCubic',
                update: () => { totalOrdersEl.innerHTML = orderCounter.value; }
            });
        }
        if (totalSpentEl) {
            let spentCounter = { value: 0 };
            anime({
                targets: spentCounter, value: totalSpent, round: 1, duration: 2000, easing: 'easeInOutCubic',
                update: () => { totalSpentEl.innerHTML = `${Math.round(spentCounter.value).toLocaleString('vi-VN')} VND`; }
            });
        }
        ...
        document.getElementById('favPrintTypeRemark').textContent = getRandomElement(habitRemarks.printType).replace(/\[CACH_IN_UU_THICH\]/g, favPrintType);
        document.getElementById('totalPagesPrintedRemark').textContent = getRandomElement(habitRemarks.totalPages).replace(/\[TONG_SO_TRANG\]/g, totalPagesPrinted.toLocaleString('vi-VN'));

        let remarkCategory = totalOrders <= 3 ? 'low' : (totalOrders <= 10 ? 'medium' : 'high');
        document.getElementById('remarkTitle').textContent = "Đôi Lời Nhắn Gửi Đặc Biệt...";
    ...
    // NÂNG CẤP: Thay thế hàm showSlide cũ bằng phiên bản có hiệu ứng
    function showSlide(index) {
        ...
        slides.forEach((slide, i) => {
            if (i === index) {
                ...
                tl.add({
                    targets: slide, opacity: [0, 1]
                }).add({
                    targets: `#${slide.id} h2 .letter`,
                    translateY: [-40, 0], opacity: [0, 1], delay: anime.stagger(35)
                }, '-=700')
                .add({
                    targets: `#${slide.id} .icon-large`,
                    scale: [0.3, 1], opacity: [0, 1]
                }, '-=800')
                .add({
                    targets: `#${slide.id} p, #${slide.id} .stat-card, #${slide.id} .habit-stats-initial`,
                    translateY: [30, 0], opacity: [0, 1], delay: anime.stagger(120)
                }, '-=600');

                if (index === slides.length - 1) {
                    launchFireworksAnime(); // Sử dụng hàm pháo hoa mới
                }
            } else {
    ...
    // NÂNG CẤP: Hàm bắt đầu có hiệu ứng chuyển cảnh mượt mà
    async function startWrappedExperience(name) {
        ...
        try {
            ...
            if (!response.ok) {
                const errorText = response.status === 404 ? `Rất tiếc, không tìm thấy thông tin cho "${name}".` : `Lỗi ${response.status}.`;
